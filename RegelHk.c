...
void RegelHk(void)
{
	...
	{	
		...
		if ( hk == HK1 )
		{
			AA_UNI[U2]->awert = pHks->Sollstell;	// HK-Sollwert-Ausgabe an Fremdregler 
			AA_UNI[U3]->awert = pHks->Vstell;			// Ausgabe an Fremdregler z.B. Wärmepumpe
		}
	...
	{	
		pHkd->vorra = wwd[WW1].wwvor;		// ***AnFre && wwd[WW1].zvorrad;
...
// ***AnFre	06.08.2009	
			pHkd->wwvorKorr = wws[WW1].TwwHkAbsenk;	// HK1 Absenkung bei WW-Vorrang
// gleitenden Absenkwert errechnen
//***AnFre		pHkd->wwvorKorr = (int) ( (float)wwd[WW1].ei_ww * ((float)pHks->Kpww / 100) );

//***AnFre			#if SP_WW1 == 0
//***AnFre				if ( pHkd->wwvorKorr > 100 )
//***AnFre					pHkd->wwvorKorr = 100;		// Siefke : bei WW-Bereitung ohne Speicher Absenkung auf 10 K begrenzt
//***AnFre			#endif												// dafuer ohne zeitl. Begrenzung moeglich
//***AnFre																		// hier : Wiedereinschalten nach TvorrZeit									
		}
	}	
		...
		{ 
			...
			{ 
			 	...
			 	{	// keine Abschaltbedingung
					if ( pHkd->ExtAnf1Hk.messw > pHkd->tvsb )
					{
						if ( hkdSoL[HK1].hkSolJa > 0 )
						{
							pHkd->tvsb = pHkd->ExtAnf1Hk.messw;
						}
						else
						{
...
// ---- Anforderung externe HAST2 ------- ***AnFre 06.06.2012 ---------------
		if ( pHks->ExtAnf2_Ja > 0)
		{
			...
			{ 
			 	if ( hkAbschalten == 0 )
			 	{	// keine Abschaltbedingung
					if ( pHkd->ExtAnf2Hk.messw > pHkd->tvsb )
					{
						if ( hkdSoL[HK1].hkSolJa > 0 )
						{
							pHkd->tvsb = pHkd->ExtAnf2Hk.messw;
						}
						else
						{
...
//			hkdSoL[HK1].solLadung_war = 0;	//für Drehzahl Pumpennachlauf löschen
...
// ***AnFre 16.11.2010 HK-Sollwert-Ausgabe auf R37_2 AA2 
	if ( hk == HK1 && pHks->Haut == 0 )
	{// Aus berechneter Temperatur(tvsb) die Steuerspannung(y) ermitteln
		// Eckpunkte X2 X1
		x2 = pHks->Tvsol10;				// SollTemperatur bei 10,0V
		x1 = pHks->Tvsol0;				// SollTemperatur bei  0,0V
		// y = mx + n			mit m = 10V-0V / X2-X1 		und n = (0V * X2 - 10V * X1) / X2-X1
		y = (int)(((long)1000 * (pHkd->tvsb - x1)) / (x2 - x1));
		if(y > 1000)	// > 10,00 V ?
			y = 1000;
		if(y < 0)
			y = 0;	
		AA_UNI[U2]->awert = y;	// HK-Sollwert-Ausgabe an Fremdregler 
	}
	...
	if ( ssfEinAnl == TRUE )							// Abgasmessung
	{
		if ( hkAbschalten )
		{
			...
			if ( hk == HK1 && pHks->Haut == 0 )
			{
				AA_UNI[U2]->awert = 0;	// HK-Sollwert-Ausgabe an Fremdregler 
			}
...
// ***AnFre 21.01.2010 ----------------------------------------------
// für die Regelung ist tvsb_solar gültig
...
// Absenkung Regel-Sollwert bei Solarer-Heizung HKS: 
	...
	// Dämpfung des berechneten Sollwertes: tvsb_solar --> gedämpfte tsol		***AnFre
	if( (pHkd->regstart == 0) || (pHkd->wwvorKorr > 0) )	// nach Reset oder bei gleit. WW-Vorrang
	{	 pHkd->tsol			= pHkd->tvsb_solar;									// keine Filterung
		 pHkd->fl_tsol	= (float)pHkd->tsol / 10;
		 pHkd->regstart	= 1;
	}	 
	else
	{
		pHkd->fl_tsol = g_filter(pHkd->fl_tsol, pHkd->tvsb_solar, pHks->Ts, pHks->Fzk);	// ***AnFre tvsb_solar
	...
	{	
		...
		if ( pHks->LeistBegrTs > 0 )
		{
			...
			{ // Ventil fährt nicht weiter Auf
				if ( fl_dy_rel > 0 ) 
					fl_dy_rel = 0;
			}
			else	if ( grenzLeistung.begrenzung == 2 )
			{ // Ventil fährt pro Tastzeit LeistBegrTs % Zu 
...
// -------- ***AnFre 12.02.2009 'Solare Heizunterstützung für HK1' -----------
	// Konventionelles Ventil:Regel-Beginn erst wenn RVENTSO[HK1] "AUF"	ist
			...
			if ( hk == HK1 )
			{
				AA_UNI[U3]->awert = pHkd->y_rel;	// Ausgabe an Fremdregler z.B. Wärmepumpe
			}
		}
	}	
...
// -------- ***AnFre 04.11.2009 STW-Abschaltung -----------
	if ( STWHK[hk]->bwert == TRUE )
	{
		...
		if ( hk == HK1 )
		{
			AA_UNI[U3]->awert = 0;	// Ausgabe an Fremdregler z.B. Wärmepumpe
		}
	}		
}
...
void rueckl_begr(char hk, char ps)
{
	...
	{	hkd[hk].rlbegr = 1;					// Statusanzeige Rücklaufbegrenzung setzen
		...
		hkd[hk].rlKorr = (int)(fl_absenk *10 );					// ***AnFre für Anzeige in hk1.h: Korrekur RL
