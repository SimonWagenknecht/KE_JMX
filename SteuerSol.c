...
// AnFre 03.08.2011 in den RAM verlegt:	int ts6warm, ts7kalt;
	int tw2Max, tw3Max;					// bei TWE-Solltemp. > 70,0°C ==> TWxMax = 75,0°C
...
// 10 Sekunden-Takt:  CO2-Einsparung berechnen aus Wmeng(3) ZIN09 
...
// Zuordnug der Temperaturen
	tw1 = TW1_[WW1]->messw;	//TWW-Regeltemp
	tw2 = TW2_[WW1]->messw;	//Speicher Oben
	tw3 = TW3_[WW1]->messw;	//Speicher Mitte
	tw4 = TW4_[WW1]->messw;	//Speicher Unten
...
//-------------- Solare Heizunterstützung ein/aus-schalten !!! jetzt in RegelHkSoL.c !!! ------------------------------
		
// --------------- Legionellenschaltung -----------------------------------------------
	if ( sod[SO1].zLegPause > 0 )
		sod[SO1].zLegPause --;
	if ( sod[SO1].zLegSperr > 0 )
		sod[SO1].zLegSperr --;
	if ( wwd[WW1].konvLegio == TRUE || wwd[WW1].solarLegio == TRUE )
		wwd[WW1].legio = 1;
	else
		wwd[WW1].legio = 0;
	
//	if(wws[WW1].Legjn == TRUE) Abhängig von WW-LEGIO-Schalter ???
//	{
	if(wwd[WW1].solarLegio == 0)			// Legio läuft nicht
	{	if( ((ts1 > sos[SO1].TS1Leg) || (ts2 > sos[SO1].TS1Leg + 20)) && (ts6warm > sos[SO1].TS6Leg)	// Temperaturen abfragen
			&& ( sod[SO1].zLegPause == 0) ) 					//Pause zwischen Solare Legion. 
		{ sod[SO1].zSLeg  = wws[WW1].Legdau * 2;	// Zähler setzen (1 min bei 30s-Task)
			wwd[WW1].solarLegio = 1;										// Temperaturerhöhung ein
		}	
	}	 
	else																		// Legio EIN, Zähler läuft
	{	if( ((ts1 > (sos[SO1].TS1Leg-50)) || (ts2 > (sos[SO1].TS1Leg-30))) || (ts6warm > (sos[SO1].TS6Leg-50)) )
		{	if( sod[SO1].zSLeg == 0 )						// wenn abgelaufen
				wwd[WW1].solarLegio = 0;
			else
				--sod[SO1].zSLeg;
			
			if( sod[SO1].zSLeg < 50 )							// wenn fast abgelaufen
			{	sod[SO1].zLegPause = sos[SO1].LegPause * 120;
				sod[SO1].zLegSperr = wws[WW1].LegSperr * 120;
			}
		}		
		else
		{	wwd[WW1].solarLegio = 0;
...
//	}
//	else Abhängig von WW-LEGIO-Schalter ???
//	{	wwd[WW1].solarLegio = 0;
//		sod[SO1].zSLeg  = 0;
//		sod[SO1].zLegPause = 0;
//	}
...
/* ---------------------------------------------------------------- */
	...
	//'Wartezeit' ist abgelaufen
		if ( (ts1 < sos[SO1].TSolEnd) && (ts2 < sos[SO1].TSolEnd + 20) )
	...
	//  WW-Ladebetrieb EIN ?
		if ( (wwd[WW1].wwLadS == TRUE) || (wwd[WW1].solarLegio == TRUE) )
		{//Es ist  Ladebetrieb oder Legionellen	
			if ( ts2 > (tw4 + sos[SO1].DTSoWwL) )
			{//Ladung durch Kollektor möglich
				SolarKollLad ();
				goto SolarEnde;
			}
			else
			{
				if ( (ts1 > (tw4 + sos[SO1].DTSoWwL + sos[SO1].DTKoll)) || (ts2 > (tw4 + sos[SO1].DTSoWwL + sos[SO1].DTKoll + 20)) )		//wenn TS1 > TW4 + 7,5 K
				{	//Ladung durch Kollektor möglich
					SolarKollEin ();
					goto SolarEnde;
				}

				// Ladung durch Puffer möglich ?
				if ( ts6warm < (tw4 + sos[SO1].DTSoWwL + sos[SO1].DTPuff) )
				{//keine solare Ladung möglich
					SolarKollEin ( );
					if ( MVWLEA[SO1]->wert == 1 )	// WW-Ladeventil offen
					{
						UVWWEA[SO1]->wert = 0;			// UmschaltVentil TWW-Solar 0: B->AB 
					}
					goto SolarEnde;
				}
				else
				{// Umschaltung auf PufferBetrieb
					sod[SO1].zPuffEin = 0;
					SolarPuffLad ();
					goto SolarEnde;
				}
		...
		{//Es ist KEIN Ladebetrieb	
			if ( ts2 > (tw2 + sos[SO1].DTSoWwZ)&& ts2 > (tw4 + sos[SO1].DTSoWwL) )
			{	//Zirk.Ausgleich durch Kollektor möglich
				SolarKollLad ();
				goto SolarEnde;
			}
			if ( (ts1 > (tw4 + sos[SO1].DTSoWwL + sos[SO1].DTKoll)) || (ts2 > (tw4 + sos[SO1].DTSoWwL + sos[SO1].DTKoll + 20)) )		//wenn TS1 > TW4 + 7,5 K
			...
			// Zirk.Ausgleich durch Puffer möglich ?
			if ( ts6warm < (tw2 + sos[SO1].DTSoWwZ + sos[SO1].DTPuff)|| ts6warm < (tw4 + sos[SO1].DTPuff) )
			{//kein solarer Zirk.Ausgleich möglich
				SolarKollEin ();
				if ( MVWLEA[SO1]->wert == 1 )	// WW-Ladeventil offen
				{
					UVWWEA[SO1]->wert = 0;			// UmschaltVentil TWW-Solar 0: B->AB 
...
/* ---------------------------------------------------------------------------------- */
	...
	{	// Kein KollektorBetrieb *************************************
		...
		{// und Kein Pufferbetrieb: Solar abschalten und Abfrage, ob wieder möglich
	...
	//'Wartezeit' ist abgelaufen
			...
			if ( ts1 > (ts6warm + sos[SO1].DTKoll) ||
						(wwd[WW1].wwLadS == 1 && ts1 > (tw4+sos[SO1].DTSoWwL+sos[SO1].DTKoll+50)) ||
						(wwd[WW1].wwLadS == 0 && ts1 > (tw4+sos[SO1].DTSoWwL+sos[SO1].DTKoll+50) && ts1 > (tw2+sos[SO1].DTSoWwZ+sos[SO1].DTKoll+50)) )
			{ //Umschalten auf Kollektorbetrieb
				if ( BusPuPara[PU_BUS_PUF-1].Funktion == 1 )		// ***AnFre Wilo-PumpenBus
					PUPUFEA[SO1]->wert = 1;		// Relais immer Ein
				else
					PUPUFEA[SO1]->wert = 0;		// Pufferpumpe aus
				sod[SO1].pufferPu	=	0;			// Wilo-PumpenBus
				sod[SO1].zKollEin = 0;
				if ( (wwd[WW1].wwLadS == 1) ) // 26.03.2008	|| (wwd[WW1].NormUnt_Sol == 1) )
				SolarKollLad ();
				else
				SolarKollEin ();
				goto SolarEnde;
...
/* ------------------------------------------------------------------------------------------ */
SolarEnde:
// Anzeigen für Parli Gruppe TWW:
	// solare WW-Vorwärmung
	if ( UVWWEA[SO1]->wert == 0	&& (sod[SO1].kollEin == 1 || sod[SO1].puffEin == 1) )
		wwd[WW1].solWwVW = 1;
	else
		wwd[WW1].solWwVW = 0;

	// solare Ladung
	if ( wwd[WW1].wwLadS == 1 && wwd[WW1].solWwLad == 1 )
		wwd[WW1].solWwLdg = 1;
	else
		wwd[WW1].solWwLdg = 0;
			
	// solare Zirk.-Ausgleich
	if ( wwd[WW1].wwLadS == 0 && wwd[WW1].solWwLad == 1 )
		wwd[WW1].solWwZir = 1;
	else
		wwd[WW1].solWwZir = 0;
...
//Solar-Abschaltungen bei zu hohen Temperaturen
	...
	// Speicher Max, "dynamisch"	19.02.2014	
	tw3Max = sos[SO1].TW3Max;
	if ( wws[WW1].Twwsol > 700 && tw3Max < 750 )
		tw3Max = 750;
		
	if ( tw3 > tw3Max )								//Speicher zu warm ?
		speicherMax = 1;
	else	if ( tw3 < tw3Max - 30 )		// 07.05.2009 Hysterese 3,0 K
		speicherMax = 0;

	// Zirkulation Max, "dynamisch"	19.02.2014
	tw2Max = sos[SO1].TW2Max;
	if ( wws[WW1].Twwsol > 700 && tw2Max < 750 )
		tw2Max = 750;
		
	if ( (tw2 > tw2Max) || (TW2_[WW1]->stat != 0) )								// VL-Zirk. zu warm ?
		zirkMax = 1;
	else	if ( (tw2 < tw2Max - 30) && (TW2_[WW1]->stat == 0) )		// 07.05.2009 Hysterese 3,0 K
		zirkMax = 0;
...
// Bei konv. Legionelle UV TWW-Solar (22) = 1:  A->AB Solartauscher-Weg
// damit Tauscher Solar-TWE (14) mit durchwärmt wird
	if ( speicherMax == 0 && wwd[WW1].konvLegio == 1 )
	{
		UVWWEA[SO1]->wert = 1;
		if ( sod[SO1].solarPu  == 1 )
		{ // es ist Solarbetrieb
			if ( ts2 < (tw4 + sos[SO1].DTSoWwL) )
			{
				UVWWEA[SO1]->wert = 0;		// AB-->B Auskühlung vermeiden
			}
		}
	}
...
// auch WILO ModBus
	...
	{ 
		...
		{
			PUPUFEA[SO1]->wert = sos[SO1].PufPuea;										// Pufferpumpe (28) ein/aus
		}
		else
		{
