...
int k_dummy;
char anz_dummy;	// ***AnFre für Solar-Anzeigetafel
...
char	so_vis[SOMAX];		// Solar ***AnFre
char	hkSoL_vis[HKSoLMAX];	// Solare Heizung		***AnFre
...
char	wilo1;				// Pumpen-Index 1: Solarpumpe
char	wilo2;				// Pumpen-Index 2: Pufferpumpe
char	wilo3;				// Pumpen-Index 3: Ladepumpe
char	wilo4;				// Pumpen-Index 4: Tauscherpumpe (oder Netzpumpe)
char	wilo5;				// Pumpen-Index 5: Zirkpumpe
...
// ***AnFre Station SEZ 40-200 (Keine Felder, Zeiger auf 1 Wert)
mwsp *TW1_[1];
mwsp *TW2_[1];
mwsp *TW3_[1];
mwsp *TW4_[1];
...
mwsp *TS6_[1];
mwsp *TS7_[1];
// ***AnFre 31.10.2007: tatsächlich benutzte Temp.werte   (wegen ev. Temperaturfehler)
int ts1;
int ts2;
int ts3;
int ts4;
...
int th2;	//***Anfre 06.06.2012
int th3;	//***Anfre 06.06.2012
int th4;	//***Anfre 06.06.2012
int th5;	//***Anfre 06.06.2012
...
//***AnFre folgende 4 Temp. nicht benutzt ! (nur für Pool-Solar)
mwsp *TKOLL[SOMAX];			// Temperatur am Solar-Kollektor
mwsp *TPUF1L[SOMAX];			// Solar-Puffer-Temp. 1 oben
mwsp *TPUF2L[SOMAX];			// Solar-Puffer-Temp. 2 unten
mwsp *TWWCSO[SOMAX];			// Regeltemp.am Solar-Übertrager
...
dinpsp	*BMPUKE[KEMAX];			// Betriebsmeldung Kessel/Zubringer-Pumpe ***AnFre
...
// ***AnFre
dinpsp	*BMPUSOL[SOMAX];		// Betriebsmeldung Solar-Pumpe
dinpsp	*BMPUPUF[SOMAX];		// Betriebsmeldung Puffer-Pumpe
dinpsp	*BMPUNT[NTMAX];			// Betriebsmeldung Netz-Pumpe sekundär
...
aasp *RVENTSO[HKSoLMAX];			// Regelventil Solare Heizunterstützung
// ***AnFre
aasp *MVSOLWW[SOMAX];					 // MotorVentil Solar-WW
aasp *MVKALTW[SOMAX];					// MotorVentil Kaltwasser
...
doutsp	*PUKOLEA[SOMAX];			// Solar-Kollektor-Pumpe		***AnFre
doutsp	*PUSOEA[SOMAX];				// Solar-Pumpe
//ulsch
doutsp	*KLKEZU[KEMAX];				// Kesselabsperrklappe ZU
doutsp	*KLKEAUF[KEMAX];			// Kesselabsperrklappe AUF
// ***AnFre
doutsp	*PUSOLEA[SOMAX];			// Solar-Kreis-Pumpe
doutsp	*PUPUFEA[SOMAX];			// Solar-Puffer-Pumpe
doutsp	*MVKOLEA[SOMAX];			// ***AnFre MotorVentil Kollektor 
doutsp	*MVPUFEA[KEMAX];			// ***AnFre MotorVentil Kessel/Puffer 
doutsp	*MVWLEA[WWMAX];				// ***AnFre MotorVentil Ladepumpen 
...
doutsp	*UVHKEA[KEMAX];			// ***AnFre 31.01.2008 Umschalt-Ventil HK1: Kessel/Puffer 
doutsp	*UVWWEA[WWMAX];			// ***AnFre 23.04.2009 Umschalt-Ventil TWW-Solar 


doutsp	**dopadr;							// Arbeitsspeicher

//---------------------------------------------------------------------------------------------
// Speicherplätze für R37-In/Out-Modul und R38-PT1000-Modul und R39-INP-Modul und R33 OUT-Modul
//---------------------------------------------------------------------------------------------
...
char	sowi;				// 1 = Sommer
// ***AnFre
char	SomBegin[2];// Datum Sommer-Anfang [Tag,Monat]
char	SomEnd[2];	// Datum Sommer-Ende	 [Tag,Monat]

char	Klima;			// Klimaschutz
int		DTKlima;		// Absenkung Klima
...
/*--------------- Reglerparameter für Solare Heizkreise  ***AnFre---------------*/
// Statische Parameter (mit Standdardwerten)	
HkSoLStandard	hksSoL[HKSoLMAX];	
// Dynamische Parameter
HkSoLDynam		hkdSoL[HKSoLMAX];
...
SoStandard	sos[SOMAX];
SoDynam	sod[SOMAX];
// ***AnFre 
char NotBetr;		//Anzeige Notbetrieb
UINT NotTage;		//Anzahl Tage bis Notbetrieb
// ***AnFre 
char zirkMax;	//Anzeige WW-Zirkulation zu warm
char speicherMax;	//Anzeige WW-Speicher zu warm
char pufferMax;	//Anzeige  Solar-Puffer zu warm
char pusoTempAl;		// Solarpumpe Temp.Alarm
char v2MinAlarm;	//Anzeige Koll.Vol.Strom minimal
char pusoFrostAl;		// Solarpumpe Frost-Alarm
// ***AnFre in Alarme.c als static definiert char zVolAlarm;		// Zähler bis Alarm EIN
char kwzDefekt;		// Kaltwasser-Zähler defekt
// ***AnFre externe HAST-Alarme
char ext1SammelSM;	//Anzeige EXT 1: EINZEL-Alarm HKV
char ext2SammelSM;	//Anzeige EXT 2: EINZEL-Alarm WPU
char ext3SammelSM;	//Anzeige EXT 3: EINZEL-Alarm KES
// ***AnFre 30.05.2013 externe SSM: Bezeichnung ändern
char extAlarmText[16];
...
UINT TH2MittelZaehler;			// Minuten-Zähler Mittlere TH2 diesen Monat (max. 44640)
UINT TH2Mittel[12];					// Mittlere TH2 Jan. bis Dez.
UINT TH2MittelVJ[12];				// Mittlere TH2 Vorjahr Jan. bis Dez.
ULONG TH2MittelSumme;
char TH2MittelMonAlt;				// Mittlere TH2 alter Monat
char TH2MittelJahrAlt;			// Mittlere TH2 altes Jahr
// ***AnFre 06.06.2012 RL Netz: TN2
UINT TN2MittelZaehler;			// Minuten-Zähler Mittlere TH2 diesen Monat (max. 44640)
UINT TN2Mittel[12];					// Mittlere TH2 Jan. bis Dez.
UINT TN2MittelVJ[12];				// Mittlere TH2 Vorjahr Jan. bis Dez.
ULONG TN2MittelSumme;
char TN2MittelMonAlt;				// Mittlere TH2 alter Monat
char TN2MittelJahrAlt;			// Mittlere TH2 altes Jahr
#endif	
...
// ***AnFre:
zaehlspVolstr volstrCtr;	//Volumenzähler Kaltwasser TWE
// ***AnFre 03.09.2009 Diagnose KWZ
diagnoseKWZ kwd;											// dyn. Parameter 5 Zeiten in h
diagParamKWZ kws;											// stat. Parameter für 4 Parameter(16 Byte)
char ZeitLoesch; 											// Zeiten löschen (auch bei KS66=1)
ULONG timeDiff;			// Zeit-Zaehler 100ms
ULONG lasttimeDiff;	// Zeit-Zaehler letzter Impuls 100ms


UINT	CO2_Faktor;				// CO2-Faktor 0,000 [kg/kWh]
ULONG	CO2_Einspar;			// CO2-Einsparung in 0 kg
char	Tyfocor;					// Anteil Tyfocor in Vol.% (nur 40, 50 oder 60)
// ***AnFre 30.04.2014: Korrekturwert für WZx Vorlauf- Rücklauf-Temperatur	[ 0,1 K ]
// spezielle Behandlung in WmengSolar.c
mwsp	WZxTV[4];					//  WZx: Vorlauf zur Anzeige
int		WZxTVKorr[4];			//	WZx: Vorlauf-Korrektur
int		WZxTVKorrAlt[4];
mwsp	WZxTR[4];					//  WZx: Rücklauf zur Anzeige
int		WZxTRKorr[4];			//	WZx: Rücklauf-Korrektur
int		WZxTRKorrAlt[4];

// ***AnFre: R101-Copy entfernt
//mwsp SSM_Ext1;						// HAST 1: Sammelstörung 				von UST1  über Bus	(R101-Copy)
//mwsp ANF_Ext1;						// HAST 1: Sollwert-Anforderung von UST1  über Bus	(R101-Copy)
//mwsp SSM_Ext2;						// HAST 2: Sammelstörung 				von UST2  über Bus	(R101-Copy)
//mwsp ANF_Ext2;						// HAST 2: Sollwert-Anforderung von UST2 über Bus	(R101-Copy)

//  Empfangsdaten von Daten-Manager	***AnFre 19.04.2012
// Sollwert-Anforderung
mwsp DM_ANF_Ext1;					// HKV 1: Sollwert-Anforderung von UST1 über S1/S2
mwsp DM_ANF_Ext2;					// HKV 2: Sollwert-Anforderung von UST2 über S1/S2
mwsp DM_SSM_Ext1;					// HKV 	: Sammelstörung 				von UST1 über S1/S2
mwsp DM_SSM_Ext2;					// WPU 	: Sammelstörung 				von WPU über S1/S2
...
mwsp ExtAnf1;	// von Unterstation 1
mwsp ExtAnf2;	// von Unterstation 2
