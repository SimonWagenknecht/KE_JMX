...
void DManager(void)
{
	...
	//*******************************
	// 3. Slave 	Sendepuffer füllen	***AnFre 02.10.2012 Adresse 13 KES
	//*******************************
	#if	DM_SLAVES > 2
	idx = 2;
	// Erster Arbeitsschritt: muss sein !
	...
	offs = DM_Fill_TxBuf(idx, offs, P&ked[KE1].tvsb,		 US_INT);									// Anforderung (Sollwert) an KES
	offs = DM_Fill_TxBuf(idx, offs, P&PUKEEA[KE1],	 JANEIN_FORMOP);							// Kessel-Freigabe
	...
	//************************************
	// 1. Slave 	Empfangsdaten behandeln	***AnFre 02.10.2012 Adresse 11 HKV
	//************************************
	idx  = 0;		// Slave-Index
	offs = 0;		// Pufferzeiger
...
// Soll-Temperatur von UST1 HeizKreisVerteiler	(Anforderung)
	...
	//************************************
	// 2. Slave 	Empfangsdaten behandeln	***AnFre 02.10.2012 Adresse 12 WPU
	//************************************
	#if	DM_SLAVES > 1
	idx  = 1;		// Slave-Index
	offs = 0;		// Pufferzeiger
	...
	//************************************
	// 3. Slave 	Empfangsdaten behandeln	***AnFre 02.10.2012 Adresse 13 KES
	//************************************
	#if	DM_SLAVES > 2
	idx  = 2;		// Slave-Index
	offs = 0;		// Pufferzeiger
	...
	offs = DM_Empty_RxBuf(idx, offs, &DM_SSM_Ext3, US_CHAR);		// 1 Byte
