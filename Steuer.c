...
#if GENI > 0	
		hkd[hk].busPuEin = pu_control[pu].pu_operation;	
#endif

#if WILO > 0	
		hkd[hk].busPuEin = modb_data[pu].output_value1_temp;	
#endif
...
// Pumpenstörung in den Parametergruppen (HKn, WW1, KE1, NT1, SOL, ...)
#if PU_BUS_HK1 > 0 
	pu = PU_BUS_HK1 - 1;
...
void PumpenbusParli ( void )
{
	...
	{	
		...
		wwd[WW1].puwtEinAnz = modb_data[PU_BUS_TPU_WW1 - 1].output_value1_temp;	// Betriebsart Ein/Aus Befehl nach Pumpe
		if ( modb_data[PU_BUS_TPU_WW1 - 1].Operation_Input < 4 )								// Operation_Input = 4: Keine Bus-Pumpe 
			wwd[WW1].puwtBmAnz	 = modb_data[PU_BUS_TPU_WW1 - 1].Operation_Input;	// Betriebsart Ein/Aus Meldung von Pumpe
		else
			wwd[WW1].puwtEinAnz	 = 0;
		wwd[WW1].puwtSmAnz	 = wwd[WW1].busTPuSm;																// Störungs-Meldung von Pumpe
		wwd[WW1].puwtLzAnz	 = modb_data[PU_BUS_TPU_WW1 - 1].op_hours;					// Pumpen-Betriebszeit
	}
	else
	{
	...
	{	
		...
		wwd[WW1].puwzEinAnz = modb_data[PU_BUS_ZPU_WW1 - 1].output_value1_temp;	// Betriebsart Ein/Aus Befehl nach Pumpe
		if ( modb_data[PU_BUS_ZPU_WW1 - 1].Operation_Input < 4 )								// Operation_Input = 4: Keine Bus-Pumpe 
			wwd[WW1].puwzBmAnz	 = modb_data[PU_BUS_ZPU_WW1 - 1].Operation_Input;	// Betriebsart Ein/Aus Meldung von Pumpe
		else
			wwd[WW1].puwzEinAnz	 = 0;
		wwd[WW1].puwzSmAnz	 = wwd[WW1].busZPuSm;																// Störungs-Meldung von Pumpe
		wwd[WW1].puwzLzAnz	 = modb_data[PU_BUS_ZPU_WW1 - 1].op_hours;					// Pumpen-Betriebszeit
	}
	else
	{
