...
// Archiv SD-Card (definiert in archivsd.h)
//#define LIST						0x35			// Lese Verzeichnisinhalt
//#define GET_DATA				0x36			// Lese Datei
//#define PUT_INIT				0x37			// Initialisiere Handle für Datei schreiben oder Verzeichnis anlegen
//#define PUT_DATA				0x38			// Schreibe Datei 
//#define DEL_FILE				0x39			// Lösche Datei oder Verzeichnis (wenn es leer ist)
//#define FORM_CARD				0x3A			// Formatiere die SD-Card
//#define READ_ATTR				0x3B			// Lese Attribute einer Datei
//#define WRITE_ATTR			0x3C			// Schreibe Attribute einer Datei
...
// 30.05.2013 Tauscher-Pumpe
	wwd[WW1].puwth = 0;			// Tauscherpumpenlaufzeit
	bicbus ( EEPADR,	(char *)&wwd[WW1].puwth,	PUWTH1_ADR, 2, BICWR);
// 30.05.2013 Zirk.-Pumpe
	wwd[WW1].puwzh = 0;			// Zirk.Pumpenlaufzeit
	bicbus ( EEPADR,	(char *)&wwd[WW1].puwzh,	PUWZH1_ADR, 2, BICWR);
#endif

// Solarpumpe und Puffer-Pumpe
#if SOANZ > 0
	sod[SO1].lzKolPuH = 0;
	bicbus ( EEPADR,	(char *)&sod[SO1].lzKolPuH,	SOLH1_ADR, 2, BICWR);
//  ***AnFre
	sod[SO1].lzPufPuH = 0;
	bicbus ( EEPADR,	(char *)&sod[SO1].lzPufPuH,	PUFH1_ADR, 2, BICWR);
#endif
...
// ext. SSM Bezeichnung		***AnFre 30.05.2013
	strcpy(extAlarmText, "EXTERNE SSM ?  ");
	bicbus(EEPADR,	(char *)&extAlarmText,	EXTALARMTEXT_ADR,		15, BICWR);
...
{	
...
#if WMENG > 0
	// Standardwert für CO2-Einsparung ***AnFre
	CO2_Faktor = 200;
	bicbus ( EEPADR,	(char *)&CO2_Faktor,	CO2_FAKTOR_ADR, sizeof(CO2_Faktor), BICWR);
	// Standardwert für Tyfocor-Anteil ***AnFre
	Tyfocor = 40;
	bicbus ( EEPADR,	(char *)&Tyfocor,	TYFOCOR_ADR, sizeof(Tyfocor), BICWR);
	...
	for ( i = 0; i <= 3; i++ )
	{
		MaxLeistung[i].MittelnLeistg = 60;
...
// ***AnFre Solar-Strahlung Vorbelegung 0 bei KaltStart 222
	sod[SO1].jahrAlt = 0;			// Jahreswechsel
	sod[SO1].monAlt = 0;				// Monatswechsel
	sod[SO1].fl_ss_Wh_g = 0;	//Gesamtwert
	sod[SO1].fl_ss_Wh_j = 0;	//Monatswert
	sod[SO1].fl_ss_Wh_m = 0;	//Monatswert
	sod[SO1].fl_ss_Wh_d = 0;	//Tageswert
	sod[SO1].fl_ss_Wh_h = 0;	//Stundenwert
	sod[SO1].SS1_kWh_vj = 0;		// SolarSensor Messung  Jahr	[ kWh/m^2 ]
	sod[SO1].SS1_kWh_vm = 0;		// SolarSensor Messung  Monat	[ kWh/m^2 ]
	sod[SO1].SS1_Wh_vd = 0;		// SolarSensor Messung  Tag	[ kWh/m^2 ]
	sod[SO1].SS1_Wh_vh = 0;		// SolarSensor Messung  Stunde	[ Wh/m^2 ]
	for ( i = 0; i < 12; i++ )
	{
		sod[SO1].SS1_Monat[i] = 0;		// SolarSensor Messung  Monate	[ kWh/m^2 ]
		sod[SO1].SS1_VJ_Monat[i] = 0;		// SolarSensor Messung  Monate Vorjahr	[ kWh/m^2 ]
	}
...
//-
	...
	// Standardwerte für Solare Heizkreise		***AnFre
	for(i = 0; i < HKSoLMAX; i++)
	{	profil = Projekte[proj_typ].hkSoLcode[i];			// Laden der Profil Nummer
		if(profil < HKSoL_PROFILE)											// auf max abtesten
			// Speicherbereich für Standardwerte laden
			memcpy( (void *)&hksSoL[i], (void *)&HkSoL_Standparam[profil], HKSSoLLENG );
	}		
...
// ***AnFre
	// Standardwerte für Warmwasser-Temperatur-Profile
	for(i = 0; i < PRMAX; i++)
	{	profil = Projekte[proj_typ].prcode[i];			// Laden der Profil Nummer
		if(profil < PR_PROFILE)											// auf max abtesten
			// Speicherbereich für Standardwerte laden
			memcpy( (void *)&prs[i], (void *)&Pr_Standparam[profil], PRSLENG );
	}		
...
void Load_Userdef()
{
	...
	dia_vis = SICHTSERV2;							// sichtbar nur ab Servicecode 2 
...
// ulsch	
	...
	// Solare Heizkreise		***AnFre
	for(i = 0; i < HKSoLMAX; i++)
	{	if(Projekte[proj_typ].hkSoLcode[i] > 0)	
		{	
			hkSoL_vis[i] = SICHTBAR;
		}	
	}
...
//***AnFre 12.12.2012 Sichtbarkeit der Anzeige Wilo-BusPumpen in den Gruppen
	wilo1 = SICHTSERV4;				// Pumpen-Index 1: Solarpumpe
	wilo2 = SICHTSERV4;				// Pumpen-Index 2: Pufferpumpe
	wilo3 = SICHTSERV4;				// Pumpen-Index 3: Ladepumpe
	wilo4 = SICHTSERV4;				// Pumpen-Index 4: Tauscherpumpe (oder Netzpumpe)
	wilo5 = SICHTSERV4;				// Pumpen-Index 5: Zirkpumpe
