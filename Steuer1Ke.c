...
{	
	...
	int kesVLmax;		// Kessel-VL Max "dynamisch": bei TWE-Sollwert > 70°C ==> VL-Max = 90°C
...
// externe HAST1
	...
	{ 
		...
		{ 
		 	if ( tvsbmax > 199 )
		 	{
				if ( pKed->ExtAnf1Ke.messw > tvsbmax )
					tvsbmax = pKed->ExtAnf1Ke.messw;
			}
			else
			{ // kein HK/WW-Sollwert
...
// externe HAST2
	...
	{ 
		...
		{ 
		 	if ( tvsbmax > 200 )
		 	{
				if ( pKed->ExtAnf2Ke.messw > tvsbmax )
					tvsbmax = pKed->ExtAnf2Ke.messw;
			}
			else
			{ // kein HK/WW-Sollwert
...
//		tvsb = kessel_rueckl_begr(KE1,tvsb);		//  Fühler nicht vorhanden !!!
	...
	// Kessel-Max "dynamisch"	19.02.2014
		kesVLmax = pKes->Tvma;
		if ( wws[WW1].Twwsol > 700 && kesVLmax < 900 )
			kesVLmax = 900;
